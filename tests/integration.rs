...
fn test_one_shot_tor_stdout() {
    ...
    child.stdin.as_mut().unwrap().write_all(b"testpassword").unwrap();
...
fn test_determinism_across_invocations() {
    ...
    let run = || {
        let mut child = Command::new(&bin)
            ...
            .spawn()
            .unwrap();
        child.stdin.as_mut().unwrap().write_all(b"same_password").unwrap();
        drop(child.stdin.take());
...
fn test_output_dir_no_overwrite() {
    ...
    let mut child = Command::new(&bin)
        .args(["--tor", "--i2p", "--validity", "3600", "--output-dir", dir.path().to_str().unwrap()])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
        .unwrap();
...
fn test_output_dir_overwrite() {
    ...
    for _ in 0..2 {
        let mut child = Command::new(&bin)
            .args(["--tor", "--validity", "3600", "--output-dir", dir.path().to_str().unwrap(), "--overwrite"])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .spawn()
            .unwrap();
    ...
    let entries: Vec<_> = std::fs::read_dir(dir.path())
        ...
        .collect();
    assert!(entries.is_empty(), "unexpected entries in output dir: {entries:?}");
}
...
fn test_validation_no_network() {
    ...
    let status = Command::new(&bin)
        ...
        .unwrap()
        .status;
    assert!(!status.success(), "should exit non-zero without --tor or --i2p");
}
...
fn test_daemon_initial_rotation_and_shutdown() {
    ...
    assert!(stdout.contains("\"event\":\"started\""), "missing started event: {stdout}");
    assert!(stdout.contains("\"event\":\"rotated\""), "missing rotated event: {stdout}");
    assert!(stdout.contains("\"event\":\"shutdown\""), "missing shutdown event: {stdout}");
    assert!(stdout.contains(".onion"), "rotated event should include onion address: {stdout}");

    // Verify all event lines are < 4096 bytes
    for line in stdout.lines() {
        assert!(line.len() < 4096, "event line too long ({} bytes): {line}", line.len());
    }
}
...
fn test_daemon_set_validity() {
...

    {
        ...
        stdin.write_all(b"{\"cmd\":\"set_validity\",\"seconds\":300}\n").unwrap();
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("\"event\":\"validity_changed\""), "missing validity_changed: {stdout}");
    assert!(stdout.contains("\"new_validity\":300"), "wrong new_validity: {stdout}");
}
...
fn test_daemon_stdin_eof_triggers_shutdown() {
    ...
    let output = child.wait_with_output().unwrap();
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("\"event\":\"shutdown\""), "missing shutdown on EOF: {stdout}");
    assert!(output.status.success());
}
